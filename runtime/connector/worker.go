// Copyright 2023 Linkall Inc.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package Connector

import (
	"context"
	"sync"

	"github.com/vanus-labs/cdk-go/connector"
	"github.com/vanus-labs/cdk-go/log"
	"github.com/vanus-labs/cdk-go/runtime/common"
)

type connectorWorker struct {
	cfgCtor       common.ConnectorConfigConstructor
	connectorCtor common.ConnectorConstructor
	config        common.WorkerConfig
	connectors    map[string]connector.Connector
	cLock         sync.RWMutex
	ctx           context.Context
	cancel        context.CancelFunc
	shuttingDown  bool
}

var _ common.Worker = &connectorWorker{}

func NewConnectorWorker(cfgCtor common.ConnectorConfigConstructor,
	connectorCtor common.ConnectorConstructor, config common.WorkerConfig) *connectorWorker {
	return &connectorWorker{
		cfgCtor:       cfgCtor,
		connectorCtor: connectorCtor,
		connectors:    map[string]connector.Connector{},
		config:        config,
	}
}

func (w *connectorWorker) Config() common.WorkerConfig {
	return w.config
}

func (w *connectorWorker) Start(ctx context.Context) error {
	w.ctx, w.cancel = context.WithCancel(ctx)
	return nil
}

func (w *connectorWorker) Stop() error {
	w.cLock.Lock()
	w.shuttingDown = true
	w.cLock.Unlock()
	w.cancel()
	var wg sync.WaitGroup
	for id := range w.connectors {
		wg.Add(1)
		go func(id string) {
			defer wg.Done()
			err := w.connectors[id].Destroy()
			log.Info().Str(log.KeyConnectorID, id).Err(err).Msg("connector destroy")
		}(id)
	}
	wg.Wait()
	return nil
}

func (w *connectorWorker) RegisterConnector(connectorID string, config []byte) error {
	w.cLock.Lock()
	defer w.cLock.Unlock()
	if w.shuttingDown {
		return nil
	}
	log.Info().Str(log.KeyConnectorID, connectorID).Msg("add a connector")
	// check the connector is existed,if true stop it
	w.removeConnector(connectorID)
	cfg := w.cfgCtor()
	c := w.connectorCtor()
	ctor := common.Connector{Config: cfg, Connector: c}
	err := ctor.InitConnector(log.WithLogger(context.Background(), log.NewConnectorLog(connectorID)), config)
	if err != nil {
		return err
	}
	log.Info().Str(log.KeyConnectorID, connectorID).Msg("connector start")
	w.connectors[connectorID] = c
	return nil
}

func (w *connectorWorker) RemoveConnector(connectorID string) {
	w.cLock.Lock()
	defer w.cLock.Unlock()
	if w.shuttingDown {
		return
	}
	log.Info().Str(log.KeyConnectorID, connectorID).Msg("remove a connector")
	w.removeConnector(connectorID)
}

func (w *connectorWorker) removeConnector(connectorID string) {
	c, ok := w.connectors[connectorID]
	if !ok {
		return
	}
	err := c.Destroy()
	if err != nil {
		log.Warn().Str(log.KeyConnectorID, connectorID).Err(err).Msg("connector stop failed")
	} else {
		log.Info().Str(log.KeyConnectorID, connectorID).Msg("connector stop success")
	}
	delete(w.connectors, connectorID)
}
