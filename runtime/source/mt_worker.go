...
var _ common.Worker = &mtSourceWorker{}

func NewMtSourceWorker(cfgCtor common.SourceConfigConstructor,
	sourceCtor common.SourceConstructor) *mtSourceWorker {
	return &mtSourceWorker{
		cfgCtor:    cfgCtor,
...
func (w *mtSourceWorker) Start(ctx context.Context) error {
	w.ctx, w.cancel = context.WithCancel(ctx)
	return nil
}

func (w *mtSourceWorker) Stop() error {
	w.cLock.Lock()
	w.shuttingDown = true
	w.cLock.Unlock()
	w.cancel()
...
func (w *mtSourceWorker) getSource(connectorID string) connector.Source {
	w.cLock.RLock()
...
func (w *mtSourceWorker) RegisterConnector(connectorID string, config []byte) error {
	w.cLock.Lock()
	if w.shuttingDown {
		return nil
	}
...
func (w *mtSourceWorker) addSource(connectorID string, sender *sourceSender) {
	w.cLock.Lock()
...
func (w *mtSourceWorker) RemoveConnector(connectorID string) {
	w.cLock.Lock()
	defer w.cLock.Unlock()
	if w.shuttingDown {
		return
